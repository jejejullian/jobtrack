...
export default function StatusBadge({ status, onChange, className = "", useFixed = false }) {
  ...
  if (!onChange) {
    return (
      <Badge variant={JOB_STATUS_BADGE[status] ?? "secondary"} className={className}>
        {status}
      </Badge>
    );
  }
  ...
  const dropdownList = (
    <ul
      role="menu"
      className={`
        z-50 min-w-28 rounded-xl border border-border bg-popover shadow-lg p-1 flex flex-col gap-0.5
  ...
  return (
    <div ref={ref} className={`relative inline-block ${className}`}>
      <Badge
        variant={JOB_STATUS_BADGE[status] ?? "secondary"}
